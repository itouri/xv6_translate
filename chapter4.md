...
![Figure4-01](images/figure4-01.JPG)
...
この状況では、idelockは永遠に解放されない -- iderwのみがロックを解放できるが、iderwはideintrが終了しない限りは実行されない --- 従って、プロセッサおよびシステム全体がデッドロックとなる。
...
例えば、プロセッサが命令AとBを順番に実行し、これらの命令の依存が無いとすると、命令Aを実行する前にまず命令Bを発行して命令Aが完了したときに、命令Bも完了させる。
